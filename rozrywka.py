...
# Rozszerzona baza RSS – dziwne fakty, absurdy, rozrywka
RSS_URLS = [
...
def validate_items(items: list) -> list:
    ...
    for item in items:
        ...
        category = str(item.get("category", "ABSRUDY ŚWIATA")).strip().upper()
        image_query = str(item.get("image_query", "funny weird fact")).strip()
...
- Ma być luźno, śmiesznie i czysto rozrywkowo. Każdy fakt ma wywoływać uśmiech lub "serio?!".
...
- "title": krótki, chwytliwy i zabawny nagłówek z unikalną emotikoną na początku
- "summary": konkretny, zabawny opis w 1-2 zdaniach
- "comment": dowcipny, sarkastyczny lub ironiczny komentarz
- "image_query": 2-4 słowa kluczowe po angielsku (np. "cute lazy panda", "funny otter", "weird historical invention")
